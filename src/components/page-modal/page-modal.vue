      ...
      >
            ...
            <template v-for="item in modalConfig.formItem" :key="item.prop">
               ...
               >
                  ...
                  <template v-else-if="item.type === 'input'">
                     <el-input v-model.lazy="formData[item.prop]" :placeholder="item.placeholder" />
                  </template>
         ...
         <template #footer>
            <span class="dialog-footer">
               <el-button type="primary" size="large" @click="enterAddUser"> 确定 </el-button>
...
function enterAddUser() {
   ...
   if (
      Object.values(formData).some(item => {
         return (typeof item === "object" && Object.keys(item as object).length === 0) || !item;
      })
   ) {
